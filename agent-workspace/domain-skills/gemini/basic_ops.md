...
Run inside browser-harness against Agent Chrome (an ISOLATED instance is
preferred — see Pitfalls; the shared 9223 instance may be busy with other
agents):

```bash
BU_NAME=gemini-test BU_CDP_URL=http://127.0.0.1:9226 browser-harness <<'PY'
...
| `close_extra_tab(fragment=None)` | Close most recently opened content tab |
...
- **Use an isolated browser instance.** The shared Agent Chrome (9223) may be
  used by other agents. For Gemini testing, copy the profile to
  `chrome-profiles/hermes-agent-gemini-test` and run on port 9226 with
  `BU_NAME=gemini-test`. Login state is preserved by copying `Default/`.
...
- **Long background watchdogs:** do not launch a long polling loop via
  `browser-harness <<'PY'` heredoc in background — it dies after the first
