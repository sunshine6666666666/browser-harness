...
| `send_message(text)` | Requires visible empty unified composer, queries the live send button immediately before activation, activates at most once, and returns `definitely_sent` only after canonical URL, empty composer, new user ID/turn, and full or safe collapsed-prefix evidence. Otherwise returns non-retryable `unknown`. |
