...
Hermes Profiles on Ye Lin's machine must not set `BU_CDP_URL` directly or copy Chrome Profiles themselves. Use the managed pool so browser selection, ownership, heartbeat, serialization, and cleanup are one operation. Omitting `--browser` resolves the registered running browser bound to fallback port 9223:
...
Tasks that require a specific managed browser select its exact top-level registry name:

```bash
browser-harness agent-pool run --browser "热点监控-9224" --site example.com --mode read <<'PY'
new_tab("https://example.com")
print(page_info())
PY
```
...
- **Managed browser safety:** Hermes tasks enter through `agent-pool run`; one lease serializes each selected browser while separate browsers remain independent. Only close tabs your own `new_tab()` returned; never batch-close by URL/domain filter. Protect work tabs with `protect_tab(...)` and check `tab_owner(tid)` before closing tabs you did not open (details in `interaction-skills/tabs.md`).
